  ...
  <section ref="simulatorRoot" class="simulator-card" :class="{ 'is-fullscreen': isFullscreen }">
    ...
    <div class="simulator-workbench">
      <div class="workbench-main">
        <details class="preset-comparison foldable-card" open>
          <summary class="foldable-card-summary">Scenario presets</summary>
        ...
        <details class="panel foldable-card" open><summary class="foldable-card-summary">Assumptions</summary><div class="control-grid">
        ...
        <details class="sensitivity-bars foldable-card" open><summary class="foldable-card-summary">Visual sensitivity</summary><div v-for="row in shipmentSensitivityRows" :key="row.changePercent" class="bar-row"><span>{{ row.changePercent > 0 ? '+' : '' }}{{ row.changePercent }}% units</span><div class="bar-track"><div class="bar-fill" :style="{ width: `${(row.result.totalHbmGb / maxShipmentPb) * 100}%` }"></div></div><strong>{{ formatCapacity(row.result.totalHbmGb) }}</strong></div><details class="table-wrap"><summary>Exact values</summary><table><tbody><tr v-for="row in contentSensitivityRows" :key="row.hbmCapacityGbPerStack"><td>{{ row.hbmCapacityGbPerStack }} GB/stack</td><td>{{ formatCapacity(row.result.totalHbmGb) }}</td></tr></tbody></table></details></details>
      </div>
